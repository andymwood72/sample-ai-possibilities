#!/usr/bin/env python3
"""
Deploy all 5 AI Team (Combined) agents to Bedrock AgentCore.

Usage:
    python deploy_all.py               # Full deploy (agents + gateway + memory)
    python deploy_all.py --agents-only # Fast update: agents only via CDK hotswap

Works on macOS, Linux, and Windows (PowerShell) without WSL.

The AgentCore Gateway (tactical-tools MCP) and Memory (team_memory) resources
are both declared in agentcore/agentcore.json and are created by `agentcore
deploy` alongside the agents. The CDK stack:
  - Builds 4 Lambda tool functions from gateway_tools/ and creates the MCP Gateway
  - Creates the AgentCore Memory resource (team_memory, 7-day event expiry)
  - Injects AGENTCORE_GATEWAY_TACTICAL_TOOLS_URL into every runtime
  - Injects MEMORY_TEAM_MEMORY_ID into every runtime

--agents-only mode:
  Skips CDK bootstrap and uses `cdk deploy --hotswap` directly, which pushes
  updated Lambda zip assets without going through a full CloudFormation update.
  Use this when you have only changed agent system prompts or main.py files and
  the gateway/memory infrastructure is already deployed. Typically 60–90 seconds
  vs 5–10 minutes for a full deploy.
...
# These two modules are staged into each agent directory at deploy time
...
print("==========================================")
print("  AI Team (Combined) — Deploy Agents")
print("==========================================\n")
if AGENTS_ONLY:
    print("  Mode: --agents-only (hotswap, skips bootstrap)")
...
try:
    ...
    # ── CDK bootstrap (idempotent, skipped in agents-only mode) ──────────────
    ...
    # ── Inject lib/ and shared modules into each agent dir ───────────────────
    ...
    print("==========================================")
    if AGENTS_ONLY:
        print("  Deploying agents via CDK hotswap")
    else:
        print("  Deploying via CDK (agentcore deploy)")
    print("==========================================")

    if AGENTS_ONLY:
        # Hotswap pushes Lambda zip updates directly — no CloudFormation update.
        # Build the CDK app first, then hotswap deploy.
        run(["npm", "run", "build"], cwd=cdk_dir)
        stack_name = f"AgentCore-TeamStrandsCombined-default"
        run(
...
if AGENTS_ONLY:
    print("  Mode:    agents-only hotswap (gateway + memory unchanged)")
else:
    print("  Gateway: tactical-tools + 4 Lambda tools (URL injected as AGENTCORE_GATEWAY_TACTICAL_TOOLS_URL)")
    print("  Memory:  team_memory, 7-day expiry (ID injected as MEMORY_TEAM_MEMORY_ID)")
print()
