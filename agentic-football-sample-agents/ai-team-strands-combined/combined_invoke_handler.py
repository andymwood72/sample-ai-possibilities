"""Invoke handler for Combined (Gateway + Memory) agents.

Wraps the agent call inside the MCPClient context so Gateway tools are
available, while memory is handled transparently by the session_manager
already wired into the Agent at creation time.
"""
...
def create_combined_invoke_handler(
    ...
    """Register the @app.entrypoint handler with Gateway MCP + Memory context."""
    log = app.logger
    ...
    async def invoke(payload, context):
        try:
            ...
            log.info(
                f"{position_label} combined agent invoked for team {team_id}, "
                f"controlling player {effective_pid}"
            )

            # MCP context provides Gateway tools; memory is wired into the agent
            with mcp_client:
            ...
            if commands:
                log.info(
                    f"LLM+tools+memory returned {len(commands)} commands: "
                    f"{[c.get('commandType') for c in commands]}"
                )
                yield json.dumps(commands)
            else:
                log.warn(
                    f"LLM parse failed, using fallback. Response: {response_text[:200]}"
                )
